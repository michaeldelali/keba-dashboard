...
export const TransactionsTable = () => {
  const totalTransactions = menuItemsData.length;

  const TableRow = (props) => {
    const { invoiceNumber, subscription, price, issueDate, dueDate, view } = props;
    const viewVariant = view === "Paid" ? "success"
      : view === "Due" ? "warning"
        : view === "Canceled" ? "danger" : "primary";
    ...
    <Card border="light" className="table-wrapper table-responsive shadow-sm">
      <Card.Body className="pt-0">
        <Table hover className="user-table align-items-center">
          ...
          <tbody>
            {menuItemsData.map(t => <TableRow key={`transaction-${t.invoiceNumber}`} {...t} />)}
          </tbody>
        </Table>
...
export const MenuTable = () => {
  ...
  const totalTransactions = menuItems.length;

  const handleMenuView = (menu) => {
    const data = menu.props;
    navigate(DefinedRoutes.ViewMenu.path, { state: data })
  }

  const handleMenuEdit = (menu) => {
      const data = menu.props;
      navigate(DefinedRoutes.EditMenu.path, { state: data })
    }

  const removeObject = (id) => {
      const filteredObjects = menuItems.filter(obj => obj.menuId !== id);
      setMenuItems(()=>filteredObjects);
    }

  const handleDelete = (id) => {
    console.log("Menu Id :: ",id);
    axiosPrivate.delete('/menu/deleteMenu', { data: {menuId:id } })
    .then((response) => {
  ...
  const TableRow = (props) => {
    ...
    const { menuId,name, description, category, price, image_url, view } = props;
    
    const viewVariant = view === "true" ? "success"
      : view === "false" ? "danger"
        : view === "" ? "warning" : "primary";
      ...
      <tr>
        ...
        <td>
          <Dropdown as={ButtonGroup}>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={()=>handleMenuView({props})}>
                  <FontAwesomeIcon icon={faEye} className="me-2" /> View
              </Dropdown.Item>
              <Dropdown.Item onClick={()=> handleMenuEdit({props})}>
                  <FontAwesomeIcon icon={faEdit} className="me-2" /> Edit
              </Dropdown.Item>
              <Dropdown.Item className="text-danger" onClick={()=>handleDelete(menuId)}>
                <FontAwesomeIcon icon={faTrashAlt} className="me-2" /> Remove
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
        ...
        <Table hover className="user-table align-items-center">
          <thead>
            <tr>
              ...
              <th className="border-bottom">View Status</th>
              <th className="border-bottom">Image</th>
              <th className="border-bottom">Action</th>
            </tr>
          </thead>
          <tbody>
            {menuItems.map(t => <TableRow key={`Menu-${t.menuId}`} {...t} />)}
          </tbody>
        </Table>
        <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
          <Nav>
